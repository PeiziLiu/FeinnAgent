...
class TestBrowserTools(unittest.TestCase):
    ...
    def test_get_browser_provider_browserbase(self):
        ...
        with patch.dict(os.environ, {"BROWSERBASE_API_KEY": "test_key", "BROWSERBASE_PROJECT_ID": "test_project"}):
            provider = _get_browser_provider()
    ...
    def test_get_browser_provider_browseruse(self):
        ...
        with patch.dict(os.environ, {"BROWSER_USE_API_KEY": "test_key"}):
            provider = _get_browser_provider()
    ...
    async def test_execute_browser_command(self, mock_get_provider, mock_get_session):
        ...
        mock_provider.execute_command.assert_called_once_with("test_session_id", "navigate", url="http://example.com")
    ...
    async def test_browser_navigate(self, mock_execute):
        ...
        result = await _browser_navigate({"url": "http://example.com"}, {"task_id": "test_task"})

        self.assertEqual(result, "Navigated to http://example.com")
        mock_execute.assert_called_once_with("test_task", "navigate", url="http://example.com")
    ...
    async def test_browser_snapshot(self, mock_execute):
        ...
        result = await _browser_snapshot({"full": True}, {"task_id": "test_task"})

        self.assertEqual(result, "Page snapshot content")
        mock_execute.assert_called_once_with("test_task", "snapshot", full=True)
    ...
    async def test_browser_click(self, mock_execute):
        ...
        result = await _browser_click({"ref": "@e5"}, {"task_id": "test_task"})

        self.assertEqual(result, "Clicked on @e5")
        mock_execute.assert_called_once_with("test_task", "click", ref="@e5")
    ...
    async def test_browser_type(self, mock_execute):
        ...
        result = await _browser_type({"ref": "@e3", "text": "test"}, {"task_id": "test_task"})

        self.assertEqual(result, "Typed 'test' into @e3")
        mock_execute.assert_called_once_with("test_task", "type", ref="@e3", text="test")
    ...
    async def test_browser_scroll(self, mock_execute):
        ...
        result = await _browser_scroll({"direction": "down"}, {"task_id": "test_task"})

        self.assertEqual(result, "Scrolled down")
        mock_execute.assert_called_once_with("test_task", "scroll", direction="down")
    ...
    async def test_browser_back(self, mock_execute):
        ...
        result = await _browser_back({}, {"task_id": "test_task"})

        self.assertEqual(result, "Navigated back")
        mock_execute.assert_called_once_with("test_task", "back")
    ...
    async def test_browser_press(self, mock_execute):
        ...
        result = await _browser_press({"key": "Enter"}, {"task_id": "test_task"})

        self.assertEqual(result, "Pressed Enter")
        mock_execute.assert_called_once_with("test_task", "press", key="Enter")
    ...
    async def test_browser_get_images(self, mock_execute):
        ...
        result = await _browser_get_images({}, {"task_id": "test_task"})

        self.assertEqual(result, "Image list")
        mock_execute.assert_called_once_with("test_task", "get_images")
    ...
    async def test_cleanup_browser(self, mock_get_provider):
        ...
        _active_sessions["test_task"] = {"session_id": "test_session_id"}
    ...
    async def test_concurrent_sessions(self):
        ...
        # Create mock provider
        mock_provider = Mock()
        mock_provider.create_session.side_effect = [{"session_id": "session1"}, {"session_id": "session2"}]
